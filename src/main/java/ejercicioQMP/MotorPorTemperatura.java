...
public class MotorPorTemperatura implements MotorDeSugerencias{

  public Stream<Prenda> obtenerPrendas(Usuario usuario) {
    return usuario.getPrendas().stream().filter(
        prenda -> {
          return prenda.getTemperaturaMaxima()
              .compareTo(ClimaServiceProvider.getInstance().obtenerTemperatura()) <= 0;
        });
  }
